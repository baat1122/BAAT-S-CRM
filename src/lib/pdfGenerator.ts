...
9. All transport-related claims, including damage, theft, or loss, must be filed directly with the carrier's insurance company. Neon Auto Transport is not responsible for carrier negligence, weather damage, vandalism, theft, or acts of God.

10. Orders canceled after dispatch may be subject to a cancellation fee. To cancel an order, customers must contact Neon Auto Transport directly before dispatch confirmation.
...
13. Pickup and delivery dates are estimated and are not guaranteed. Neon Auto Transport will work diligently to secure timely transport but cannot guarantee exact dates or times due to factors beyond our control.
...
export function generatePDF(data: any) {
  ...
  pdf.text("NEON", margin, 18);
  pdf.setTextColor(59, 130, 246);
  pdf.setFontSize(22);
  pdf.text("AUTO TRANSPORT", margin + 28, 18);
  // Tagline
  ...
  pdf.setFontSize(8);
  pdf.text("(571) 576-7711", W - margin, 15, { align: "right" });
  pdf.text("info@neonautotransport.com", W - margin, 21, { align: "right" });
  y = 40;
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    pdf.setFontSize(7);
    pdf.setFont("helvetica", "normal");
    pdf.text("Neon Auto Transport  |  (571) 576-7711  |  info@neonautotransport.com", W / 2, 291, { align: "center" });
